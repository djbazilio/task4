(function () {
  ...
  function PasswordController($scope, $stateParams, UsersService, $location, Authentication, PasswordValidator, Notification) {
    ...
    function onRequestPasswordResetSuccess(response) {
      ...
      vm.credentials = null;
      Notification.success({ message: response.message, title: '<i class="glyphicon glyphicon-ok"></i> Письмо смены пароля успешно отправлено!' });
    }

    function onRequestPasswordResetError(response) {
      // Show user error message and clear form
      vm.credentials = null;
      Notification.error({ message: response.data.message, title: '<i class="glyphicon glyphicon-remove"></i>  Не удалось отправить письмо для изменения пароля!', delay: 4000 });
    }
    ...
    function onResetPasswordSuccess(response) {
      ...
      Notification.success({ message: '<i class="glyphicon glyphicon-ok"></i> Пароль успешно сброшен!' });
      // And redirect to the index page
      $location.path('/password/reset/success');
    }

    function onResetPasswordError(response) {
      Notification.error({ message: response.data.message, title: '<i class="glyphicon glyphicon-remove"></i>  Сбой при сбросе пароля!', delay: 4000 });
    }
  }
}());
