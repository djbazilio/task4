  ...
  describe('Edit Profile Controller Tests', function () {
    ...
    describe('Update User Profile', function () {
      ...
      it('should update the user profile', inject(function (UsersService) {
        ...
        expect(Notification.success).toHaveBeenCalledWith({ message: '<i class="glyphicon glyphicon-ok"></i> Профиль изменен успешно!' });
      }));
      ...
      it('should call Notification.error if error', inject(function (UsersService) {
        ...
        expect(Notification.error).toHaveBeenCalledWith({ message: errorMessage, title: '<i class="glyphicon glyphicon-remove"></i> Изминения не сохранились!' });
      }));
    });
